        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 gap-x-12 gap-y-5 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <label className="text-[14px] font-bold ">Name of the Candidate *</label>
              <input 
                {...register("firstName")} 
              ...
              <div className="flex gap-6 text-[13px]">
                {["Male", "Female", "Neutral"].map((g) => (
                  <label key={g} className="flex items-center gap-2">
                    <input type="radio" value={g} {...register("gender")} className="accent-[#3f2b96]" />
              ...
              <div className="flex gap-6 text-[13px]">
                {["Single", "Married", "Divorced"].map((s) => (
                  <label key={s} className="flex items-center gap-2">
                    <input type="radio" value={s} {...register("status")} className="accent-[#3f2b96]" />
            ...
            <div className="space-y-1.5">
              ...
              <input {...register("currentCTC")} className="w-full border border-[#e0e0e0] p-2.5 text-[13px] outline-none" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[14px] font-bold">Expected CTC*</label>
              <input {...register("expectedCTC")} className="w-full border border-[#e0e0e0] p-2.5 text-[13px] outline-none" />
            </div>
              ...
              <div className="flex flex-wrap gap-4 text-[13px]">
                {["Fresher", "1 – 5", "6 – 10", "> 11"].map((e) => (
                  <label key={e} className="flex items-center gap-2">
                    <input type="radio" value={e} {...register("experience")} className="accent-[#3f2b96]" />
              ...
              <div className="flex flex-wrap gap-4 text-[13px]">
                {["State", "CBSE", "CISCE", "Cambridge", "IB", "Others"].map((c) => (
                  <label key={c} className="flex items-center gap-2">
                    <input type="checkbox" value={c} {...register("curriculum")} className="accent-[#3f2b96]" />
      ...
      <label key={l} className="flex items-center gap-3 cursor-pointer">
        ...
        <span>{l}</span>
            ...
            <div className="md:col-span-2 space-y-3">
              ...
              <div className="space-y-2 text-[12px] font-medium text-gray-700">
                {["Immediate Joining", "Serving Notice Period", "3 Months & Above", "Looking for Next Academic Year"].map((n, idx) => (
